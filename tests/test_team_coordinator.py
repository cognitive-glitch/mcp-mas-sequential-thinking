...
class TestTeamCoordinatorInitialization:
    ...
    async def test_initialize_teams_success(self):
        ...
        with patch.object(coordinator, "_get_team_model") as mock_team_model:
            with patch.object(
                coordinator, "_get_reflection_model"
            ) as mock_reflection_model:
                with patch.object(
                    coordinator, "_create_primary_team"
                ) as mock_create_primary:
                    with patch.object(
                        coordinator, "_create_reflection_team"
                    ) as mock_create_reflection:
                        # Mock return values
...
class TestTeamCoordinatorInstructionGeneration:
    ...
    async def test_generate_adaptive_instructions_with_context(self):
        ...
        tech_context = {"domain": "technical", "complexity": 0.8, "is_revision": False}

        tech_instructions = await coordinator.generate_adaptive_instructions(
            tech_context
        )
        ...
        creative_instructions = await coordinator.generate_adaptive_instructions(
            creative_context
        )
    ...
    async def test_adaptive_instructions_edge_cases(self):
        ...
        unknown_instructions = await coordinator.generate_adaptive_instructions(
            unknown_context
        )
        ...
        complex_instructions = await coordinator.generate_adaptive_instructions(
            high_complex_context
        )
        ...
        with patch.object(coordinator, "_get_team_model") as mock_get_model:
            with patch.object(
                ...
                with patch.object(
                    ...
                    mock_create_reflection.side_effect = Exception(
                        "Reflection team error"
                    )
