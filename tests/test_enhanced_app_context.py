...
class TestEnhancedAppContextTeamManagement:
    ...
    async def test_initialize_teams_success(self, app_context_with_provider):
        ...
        with patch.object(
            context, "_create_primary_team", return_value=mock_primary_team
        ):
            with patch.object(
                context, "_create_reflection_team", return_value=mock_reflection_team
            ):
    ...
    @pytest.mark.asyncio
    async def test_initialize_teams_already_initialized(
        self, app_context_with_provider
    ):
    ...
    @pytest.mark.asyncio
    async def test_initialize_teams_model_creation_failure(
        self, app_context_with_provider
    ):
        """Test handling of model creation failure during team initialization."""
        context = app_context_with_provider
        context.provider_config.create_model_instance.side_effect = Exception(
...
class TestEnhancedAppContextThoughtManagement:
    ...
    async def test_add_thought(self, app_context, sample_thought):
        ...
        app_context.shared_context.update_from_thought.assert_called_once_with(
            sample_thought
        )
    ...
    async def test_get_relevant_context(self, app_context):
        ...
        app_context.shared_context.get_relevant_context = AsyncMock(
        ...
        app_context.shared_context.get_relevant_context.assert_called_once_with(
            "test thought"
        )
...
class TestEnhancedAppContextInstructionGeneration:
    ...
    @pytest.mark.asyncio
    async def test_generate_adaptive_coordinator_instructions_with_history(
        self, app_context
    ):
    ...
    @pytest.mark.asyncio
    async def test_generate_adaptive_coordinator_instructions_slow_performance(
        self, app_context
    ):
