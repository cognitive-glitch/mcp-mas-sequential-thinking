...
from src.models.thought_models import (
    ThoughtData, ProcessedThought, ThoughtSequenceReview, 
    SessionContext,
    DomainType, BranchAnalysis
)
...
class EnhancedAppContext:
    ...
    def __init__(self):
        ...
        self.session_context = SessionContext(
            ...
            available_tools=["ThinkingTools", "ExaTools"],  # Will be populated dynamically
        ...
        logger.info(f"Enhanced app context initialized with session ID: {self.session_id}")
    
    async def initialize_models(self) -> None:
        """Initialize LLM models using the enhanced provider factory."""
        try:
            self.team_model, self.agent_model, self.provider_config = LLMProviderFactory.create_models()
            logger.info(f"Models initialized successfully using {self.provider_config.provider_name}")
        except Exception as e:
    ...
    async def add_thought(self, thought_data: ThoughtData) -> None:
        ...
        try:
            ...
            logger.debug(f"Thought #{thought_data.thoughtNumber} added to shared context")
...
async def process_thought_with_dual_teams(thought_data: ThoughtData) -> ProcessedThought:
    ...
    try:
        ...
        relevant_context = await app_context.get_context_for_thought(thought_data.thought)
...
**Special Handling**:"""
        ...
        logger.info(f"Processing thought #{thought_data.thoughtNumber} with primary team...")
        ...
        logger.info(f"Primary team completed processing thought #{thought_data.thoughtNumber}")
        ...
        if len(primary_content.strip()) > 50:  # Only reflect on substantial responses
            logger.info(f"Processing reflection for thought #{thought_data.thoughtNumber}...")
...
Please provide constructive reflection with specific feedback and recommendations."""
            ...
            try:
                ...
                reflection_response = await app_context.reflection_team.arun(reflection_input)
                if reflection_response and hasattr(reflection_response, 'content'):
                    ...
                    logger.info(f"Reflection completed for thought #{thought_data.thoughtNumber}")
            except Exception as e:
                logger.warning(f"Reflection team failed for thought #{thought_data.thoughtNumber}: {e}")
        ...
        # Record performance metrics
        await app_context.record_performance_metric("thought_processing_time_ms", execution_time)
        await app_context.record_performance_metric("response_length", len(integrated_response))
        ...
        processed_thought = ProcessedThought(
            ...
            token_usage={
                ...
                "reflection_team": len(reflection_content.split()) if reflection_content else 0
            },
            success=True,
        ...
        logger.info(f"Successfully processed thought #{thought_data.thoughtNumber} in {execution_time}ms")
    ...
    try:
        ...
        for branch_id, branch_data in branches.items():
            analysis = BranchAnalysis(
                ...
                recommendation="Continue development" if branch_data['quality'] > 0.6 else "Consider revision"
            )
            branch_analyses.append(analysis)
        
        # Calculate quality metrics
        performance_summary = await app_context.shared_context.get_performance_summary()
        avg_processing_time = performance_summary.get('thought_processing_time_ms', {}).get('mean', 0)
        avg_response_length = performance_summary.get('response_length', {}).get('mean', 0)
        
        # Generate insights from context
        key_insights = [insight.content for insight in app_context.shared_context.key_insights[-10:]]
        ...
        review = ThoughtSequenceReview(
            ...
            overall_quality=min(0.9, (avg_response_length / 1000) + 0.3),  # Simple quality heuristic
            ...
            tool_effectiveness={
                ...
                for tool in [tool_decision] if hasattr(tool_decision, 'confidence')
            },
            branch_analyses=branch_analyses,
            best_branch=max(branches.keys(), key=lambda x: branches[x]['quality']) if branches else None,
            next_steps=[
                "Continue with most promising branch" if branches else "Proceed with linear development",
...
async def sequentialthinking(
    ...
    try:
        ...
        return processed_thought.integrated_response + processed_thought.next_step_guidance
...
async def lifespan(app):
    ...
    try:
        ...
        provider_name = app_context.provider_config.provider_name if app_context.provider_config else "unknown"
